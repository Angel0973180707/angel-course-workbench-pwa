...
const STATE_SHEET = { idea:"發想", draft:"草稿", final:"幸福教養課程" };
...
function readSettings(){
  ...
  const rawCourse = saved.courseApi || DEFAULT_COURSE_API;
  const rawTools  = saved.toolsApi  || DEFAULT_TOOLS_API;
  const courseApi = normalizeExecUrl_(rawCourse);
  const toolsApi  = normalizeToolsApi_(rawTools);
  // auto-heal legacy/wrong settings
  if (courseApi !== rawCourse || toolsApi !== rawTools){
    localStorage.setItem("angelCourseWB.settings", JSON.stringify({ courseApi, toolsApi }));
  }
  return { courseApi, toolsApi };
}

function normalizeExecUrl_(u){
  try{
...
function normalizeToolsApi_(u){
  // expected: .../exec?sheet=工具庫存管理&format=tools
  const s = String(u||"").trim();
  try{
    const url = new URL(s);
    // if someone pasted just /exec (no params), we append
    const sheet = (url.searchParams.get("sheet") || "").trim();
    const format = (url.searchParams.get("format") || "").trim();
    // common legacy mistake: sheet=tools or missing sheet/format
    if (!sheet || sheet.toLowerCase() === "tools") url.searchParams.set("sheet", "工具庫存管理");
    if (!format) url.searchParams.set("format", "tools");
    // if format is not tools, keep it but if sheet says 工具庫存管理 we still want tools format
    if (url.searchParams.get("sheet") === "工具庫存管理") url.searchParams.set("format", "tools");
    return url.toString();
  }catch(e){
    return s;
  }
...
async function toolsFetchAll(){
  ...
  const res = await fetch(toolsApi, { method:"GET" });
  const json = await res.json();
...
async function init(){
  ...
  $("#btnTestApi").addEventListener("click", async ()=>{
    try{
      ...
      const b = await fetch(tools).then(r=>r.json());
