const LS_KEY = "angel_course_workbench_step2_1";
...
function ensureId(){ if(!model.id) model.id = "C-" + Date.now(); }
function setState(next){ model.state = next; renderAll(); }
function setDot(dotEl, status){ dotEl.classList.remove("ok","doing","todo"); dotEl.classList.add(status); }
...
function bindValue(inputId, field){
  ...
  if(!node) return;
  node.addEventListener("input", ()=>{ model[field] = node.value; model.updated_at = nowISO(); renderProgress(); });
}
function bindSelect(selectId, field){
  const node = document.getElementById(selectId);
  if(!node) return;
  node.addEventListener("change", ()=>{ model[field] = node.value; model.updated_at = nowISO(); renderProgress(); });
}
function bindInputs(){
...
function renderSteps(){
  ...
  if(s==="idea"){
    ...
    area.appendChild(stepCard({
      key:"I3", title:"I-3｜工具配方（先手填，下一步接工具庫）",
      sub:"主工具單選 / 副工具多選（下一步做「頁內勾選」）",
      bodyHtml:`
        <div class="grid two">
        ...
        <div class="hint">下一步（2-2）接上工具庫 API，你就能用搜尋＋勾選完成。</div>
      `
    }));
    area.appendChild(stepCard({
      key:"I4", title:"I-4｜粗架構（8 集一句話大綱）",
      sub:"先用最短句把 8 集站起來",
      bodyHtml:`
        ${textareaField({id:"f_framework",label:"8 集一句話大綱（framework_text）",placeholder:"01 ...\n02 ...\n...\n08 ...",value:model.framework_text})}
        <div class="row"><button class="btn primary" id="btnGoDraft">進草稿 →</button></div>
  ...
  bindInputs(); bindStateJumpButtons();
}
...
function buildTSVRow(){
  ...
  const mainToolLine = `${model.main_tool_name||""} ${model.main_tool_link||""}`.trim();
  if(mainToolLine && (model.main_tool_link||"") && !links.includes(model.main_tool_link)){
    links = [links, mainToolLine].filter(Boolean).join("\n");
  }
  const cols = [
    model.id, model.title, model.type, model.status || "ready", model.version || "v1",
    model.owner, model.audience, model.duration_min, model.capacity, model.tags,
    summary, objectives, outline, materials, links, model.assets, model.notes,
...
function bindBottomButtons(){
  ...
  el("btnCopyTSV").addEventListener("click", async ()=>{
    const ok = await copyToClipboard(buildTSVRow());
  ...
  el("btnSettings").addEventListener("click", ()=>{
    alert("下一步會加入：API 設定、預設 owner、工具庫勾選（不遮罩）。");
  });
}
function renderAll(){ renderProgress(); renderSteps(); }
function init(){ bindTopProgressButtons(); bindBottomButtons(); renderAll(); }
init();
